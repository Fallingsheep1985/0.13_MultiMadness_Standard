        ...
        if (!isNull player) then {
            ...
            if (_vehicle != _currentVehicle) then {
                ...
                if (_currentVehicle != player && _isNearFeed && !(_currentVehicle isKindof "Bicycle")) then {
                    ...
                    _vehicle_refuel_id = _vehicle addAction ["Refuel", "scripts\AutoRefuel\kh_vehicle_refuel.sqf", [_amount], -1, false, true, "", "vehicle _this == _target && local _target"];
                };
            };
            if (!_isNearFeed) then {
                _vehicle removeAction _vehicle_refuel_id;
